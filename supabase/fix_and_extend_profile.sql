...
alter table posts add constraint title_length_check check (char_length(title) between 5 and 60);

alter table posts drop constraint if exists body_length_check;
alter table posts add constraint body_length_check check (char_length(body) >= 20);

alter table profiles drop constraint if exists description_length_check;
alter table profiles add constraint description_length_check check (description is null or char_length(description) <= 300);
