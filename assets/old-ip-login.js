...
import { SUPABASE_URL, SUPABASE_ANON_KEY, IPQS_API_KEY } from "./config.js";
...
// Catégories vérifiées. NB : l'API IPQualityScore n'a pas de champ distinct pour
// "datacenter" vs "hosting" vs "cloud" — les trois sont approximés par le même
// signal connection_type === "Data Center". C'est une limite du service utilisé,
...
async function run() {
  ...
  if (!IPQS_API_KEY || IPQS_API_KEY === "COLLE_TA_CLE_IPQS_ICI") {
    scanStatus.textContent =
      "La détection VPN/proxy n'est pas configurée sur ce site (clé IPQualityScore manquante) — refusé par prudence.";
    return;
  }

  try {
    const res = await fetch(
      `https://ipqualityscore.com/api/json/ip/${IPQS_API_KEY}/${detectedIp}?strictness=1`
    );
    const data = await res.json();

    const flags = {
      vpn: !!(data.vpn || data.active_vpn),
      proxy: !!data.proxy,
      tor: !!(data.tor || data.active_tor),
      datacenter: data.connection_type === "Data Center",
      hosting: data.connection_type === "Data Center",
      cloud: data.connection_type === "Data Center",
      residential: data.connection_type === "Residential",
      mobile: !!data.mobile,
      reputation: (data.fraud_score || 0) >= 75 || !!data.recent_abuse,
      bot: (data.bot_status ?? 0) >= 50,
      bogon: false
    };
