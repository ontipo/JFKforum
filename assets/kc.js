...
function renderSponsorLink(code) {
  ...
  document.getElementById("sponsor-slot").innerHTML = `
    ...
    <div class="share-row">
      <button id="share-fb-btn" class="share-btn-icon" title="Publier sur Facebook" style="border:none;background:transparent;padding:0">
        <span class="share-btn-icon" style="width:44px;height:44px;display:flex">
      ...
      <button id="share-messenger-btn" class="share-btn-icon" title="Envoyer sur Messenger" style="border:none;background:transparent;padding:0">
        <span class="share-btn-icon" style="width:44px;height:44px;display:flex">
    ...
    <p id="share-copy-msg" class="hint-text hidden" style="margin-top:8px;color:var(--silver-300)"></p>
  `;

  const copyMsg = document.getElementById("share-copy-msg");

  // Facebook ne permet pas de pré-remplir un texte personnalisé dans sa boîte de
  // partage (limitation de leur plateforme, pas de notre code) — on copie le texte
  // pour qu'il n'ait plus qu'à le coller dans son post.
  document.getElementById("share-fb-btn").addEventListener("click", async () => {
    await navigator.clipboard.writeText(shareText);
    copyMsg.textContent = "Texte copié — colle-le dans ta publication Facebook (Facebook ne permet pas de le pré-remplir).";
    copyMsg.classList.remove("hidden");
    window.open(fbUrl, "_blank");
  });

  document.getElementById("share-messenger-btn").addEventListener("click", async () => {
    if (isMobile) {
      window.location.href = `fb-messenger://share/?link=${encodeURIComponent(link)}`;
      return;
    }
    // Sur ordinateur, aucun lien officiel ne permet de pré-remplir Messenger sans
    // application Facebook enregistrée — on copie le texte et on ouvre Messenger.
    await navigator.clipboard.writeText(shareText);
    copyMsg.textContent = "Texte copié — colle-le dans ta conversation Messenger.";
    copyMsg.classList.remove("hidden");
